...
const ProfileView: React.FC = () => {
  ...
  const [profile, setProfile] = useState({
    name: authProfile?.name || 'আরিফুর রহমান',
    age: authProfile?.age || 25,
    email: authProfile?.email || 'arif.demo@email.com',
    bio: authProfile?.bio || 'Software Engineer @ Rajshahi',
    education: authProfile?.education || 'রাজশাহী বিশ্ববিদ্যালয়',
    image: authProfile?.image || 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=400&auto=format&fit=crop&q=60',
    gallery: [] as string[],
    interests: authProfile?.interests || ['💻 Tech', '🎮 Gaming', '✈️ Travel', '🎵 Music']
  });
  ...
  useEffect(() => {
    if (authProfile) {
      setProfile({
        ...
        gallery: [], // Gallery needs separate handling if implemented
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Optimistically update local state if needed (though useEffect should catch it)
      setProfile(editForm);
    }
  };
                ...
                <div className="grid grid-cols-2 gap-2">
                  {interestList.map(interest => (
                    <button
                      ...
                      className={`p-2 rounded-lg border text-xs font-bold transition-all ${editForm.interests.includes(interest)
            ...
            {[0, 1, 2, 3].map(i => {
              ...
              return (
                <div
                  ...
                  className={`aspect-square relative rounded-2xl overflow-hidden cursor-pointer group shadow-sm hover:shadow-md transition-all ${!img ? 'bg-white dark:bg-surface-dark border-2 border-dashed border-primary/30 hover:bg-primary/5 flex items-center justify-center' : ''
                    }`}
                >
                  {img ? (
                    <>

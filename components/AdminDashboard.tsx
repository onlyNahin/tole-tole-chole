...
import { ADMIN_NOTIFICATIONS } from '../constants';
import { UserReview, UserProfile, Moderator, PremiumFeature, PremiumPlan, PremiumPermissions, SiteConfig, SystemLog, AboutPageConfig, Report, AppEvent, AdminNotification } from '../types';
...
const OverviewTab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchStats();
  }, [reports, events, tickets]); // Refresh when contexts change

  const stats = [
    { label: 'Total Users', value: totalUsers.toString(), icon: 'people', color: 'blue', trend: '+12%', trendDirection: 'up' },
    { label: 'Events Active', value: events.length.toString(), icon: 'event', color: 'purple', trend: '+5%', trendDirection: 'up' },
    { label: 'Active Reports', value: reports.filter(r => r.status !== 'Resolved').length.toString(), icon: 'report_problem', color: 'amber', trend: '-2%', trendDirection: 'down' },
    { label: 'Pending Tickets', value: tickets.filter(t => t.status === 'Open').length.toString(), icon: 'confirmation_number', color: 'green', trend: '+8%', trendDirection: 'up' },
  ];
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {reports.slice(0, 3).map((report) => (
                <tr key={report.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${report.status === 'Resolved' ? 'opacity-50 grayscale bg-gray-50/50 dark:bg-black/10' : ''}`}>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${report.reason === 'Harassment' ? 'bg-red-100 text-red-700' :
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase flex items-center w-fit gap-1 ${report.status === 'Pending' ? 'bg-yellow-100 text-yellow-700' :
                      report.status === 'Reviewing' ? 'bg-blue-100 text-blue-700' :
                        'bg-green-100 text-green-700'
                      }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${report.status === 'Pending' ? 'bg-yellow-500' :
            ...
            <div key={event.id} className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800/50">
              ...
              <div className="flex items-center gap-6">
                <div className="text-right hidden sm:block">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${event.status === 'Upcoming' ? 'bg-green-100 text-green-700' :
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {tickets.map((ticket) => (
              <tr key={ticket.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${ticket.priority === 'High' ? 'bg-red-100 text-red-700' :
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {filteredReports.map((report) => (
              <tr key={report.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${report.severity === 'high' ? 'bg-red-100 text-red-700' :
                ...
                <td className="px-6 py-4">
                  <span className={`flex items-center gap-1 text-sm font-bold ${report.status === 'Resolved' ? 'text-green-600' :
...
const AnalyticsTab: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      ...
      const { data: maleCount } = await supabase
        .from('profiles')
        .select('*', { count: 'exact', head: true })
        .eq('gender', 'Male');

      const { data: femaleCount } = await supabase
        .from('profiles')
        .select('*', { count: 'exact', head: true })
        .eq('gender', 'Female');

      setUserDistribution([
        { name: 'Male', value: maleCount ? (maleCount as any).length : 65 }, // Fallback to demo if counts are 0/empty
        { name: 'Female', value: femaleCount ? (femaleCount as any).length : 35 },
      ]);
      setIsLoading(false);
    };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-surface-dark p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
          <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-6">User Demographic Growth</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={userDistribution.length > 0 ? userDistribution : dataUser} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                  {(userDistribution.length > 0 ? userDistribution : dataUser).map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip />
...
const UserManagementTab: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingUser) {
      const interests = editingInterests.split(',').map(s => s.trim()).filter(s => s);
      const userData = {
        ...editingUser,
        interests,
      ...
      let error;
      if (isAdding) {
        // For new users, we might want to use supabase.auth.signUp if we want them to have login access
        // But for simplicity in admin panel, we just insert into profiles
        delete (userData as any).id; // Let DB generate UUID
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {filteredUsers.map((user) => (
              <tr key={user.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${selectedUsers.includes(user.id) ? 'bg-primary/5 dark:bg-primary/10' : ''}`}>
                ...
                <td className="px-6 py-4">
                  <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${user.status === 'active' ? 'bg-green-100 text-green-700' :
...
const ModeratorsTab: React.FC = () => {
  ...
  const fetchModerators = async () => {
    ...
    if (error) {
      ...
      setModerators((data || []).map(m => ({
        ...
        lastActive: m.last_active ? new Date(m.last_active).toLocaleString() : 'Never'
      } as Moderator)));
    }
    setIsLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const dataToSave = {
      ...
      permissions: formData.permissions || ['manage_content'],
      last_active: new Date().toISOString()
    };

    let error;
    if (editingId) {
      const { error: updateError } = await supabase.from('moderators').update(dataToSave).eq('id', editingId);
      error = updateError;
    } else {
      const { error: insertError } = await supabase.from('moderators').insert([dataToSave]);
      error = insertError;
    }
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {moderators.map((mod) => (
              <tr key={mod.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${mod.role === 'Senior Moderator' ? 'bg-purple-100 text-purple-700' :
...
const SettingsTab: React.FC = () => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    // updateConfig already saves to DB, so we just add a delay for effect
    setTimeout(() => {
      setIsSaving(false);
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) => updateConfig({ allowNewRegistrations: e.target.checked })}
                className="sr-only peer"
              />
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => updateConfig({ globalAnnouncement: e.target.value })}
              className="w-full rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 focus:ring-primary focus:border-primary text-gray-800 dark:text-gray-200"
      ...
      <div className="bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="number"
              value={config.freeDailySwipes}
              onChange={(e) => updateConfig({ freeDailySwipes: parseInt(e.target.value) })}
              className="w-full rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 focus:ring-primary focus:border-primary text-gray-800 dark:text-gray-200"
            />
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                value={config.premiumPlans[0]?.price || '499'}
                onChange={(e) => {
                  const newPlans = [...config.premiumPlans];
                  if (newPlans[0]) newPlans[0].price = e.target.value;
                  updateConfig({ premiumPlans: newPlans });
                }}
                className="w-full pl-8 rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 focus:ring-primary focus:border-primary text-gray-800 dark:text-gray-200 font-bold"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 focus:ring-primary focus:border-primary text-gray-800 dark:text-gray-200"
                placeholder="https://..."
              />
            </div>
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="w-full bg-primary text-white font-bold py-2 rounded-lg hover:bg-primary-dark transition-colors disabled:opacity-50"
          >
...
const PremiumManagementTab: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    if (error) {
      console.error('Error fetching users for premium:', error);
    } else {
      setUsers(data as UserProfile[]);
    }
    setIsLoading(false);
  ...
  // User Handlers
  const toggleUserPremium = async (userId: string, status: boolean) => {
    if (window.confirm(`Are you sure you want to ${status ? 'grant' : 'revoke'} premium status for this user?`)) {
      const expiry = status ? new Date() : null;
      if (expiry) expiry.setMonth(expiry.getMonth() + 1);

      const expiryStr = expiry ? expiry.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : null;
  ...
  const handleAddPremium = async (e: React.FormEvent) => {
    ...
    if (addMode === 'existing') {
      ...
      await toggleUserPremium(selectedExistingId, true);
    } else {
      // Create new user with premium (simulated as we don't have auth registration here, just profile insert)
      ...
      if (error) {
        console.error('Error adding new premium user:', error);
      } else {
        fetchUsers();
      }
    }
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {premiumUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => toggleUserPremium(user.id, false)}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {logs.map((log) => (
              <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td className="px-6 py-4 text-right">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${log.status === 'Success' ? 'bg-green-100 text-green-700' :
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(notif => (
                      <div key={notif.id} className={`p-4 border-b border-gray-50 dark:border-gray-800 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${!notif.isRead ? 'bg-primary/5' : ''}`}>
                        <div className="flex gap-3">
                          <div className={`mt-1 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${notif.type === 'user' ? 'bg-blue-100 text-blue-600' :
